...
export class CartService {
    ...
    async getLastActiveCartByUserId(userId: number): Promise<Cart | null> {
        const carts = await this.cart.find({
            where: {
                userId: userId, // lijevi userId je iz carta a desni iz metode koja vraca Promise 
            },
            order: {
                createdAt: "DESC",
            },
    ...
    async addArticleToCart(cartId: number, articleId: number, quantity: number): Promise<Cart> {
        let record: CartArticle = await this.cartArticle.findOne({where: // await uvijek koristimo u radu sa bazom 
            {
            cartId: cartId,
            articleId: articleId,
            }  
